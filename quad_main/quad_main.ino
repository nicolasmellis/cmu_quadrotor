...
#define MAGADDR (0x3D >> 1) 
#define PERSONALITYADDR 2 // 
...
void readPersonality()
{
  ...
  //Send requested register (0x10 == command)
  Wire.beginTransmission(PERSONALITYADDR);
  Wire.write(0x10);
  Wire.endTransmission();

  //Request 1 byte
  Wire.requestFrom(PERSONALITYADDR,1);
  while(Wire.available() < 1);
  command = Wire.read();
  
  //Send requested register (0x20 == magnitude)
  Wire.beginTransmission(PERSONALITYADDR);
  Wire.write(0x20);
  Wire.endTransmission();
